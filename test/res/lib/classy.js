...
;(function(undefined) {
  ...
  /* restore the global Classy name and pass it to a function.  This allows
     different versions of the classy library to be used side by side and
     in combination with other libraries. */
  Classy.$noConflict = function() {
    try {
      setOrUnset(root, 'Classy', old_class);
    }
    catch (e) {
      // fix for IE that does not support delete on window
      root.Classy = old_class;
    }
    return Classy;
  };

  /* what version of classy are we using? */
  Classy.$classyVersion = CLASSY_VERSION;

  /* extend functionality */
  Classy.$extend = function(properties) {
    ...
    rv.constructor = rv;
    rv.$extend = Classy.$extend;
    rv.$withData = Classy.$withData;
    return rv;
  };

  /* instanciate with data functionality */
  Classy.$withData = function(data) {
